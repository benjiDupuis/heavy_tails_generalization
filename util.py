...
if os.getcwd()[:21] == '/cluster/home/klehman':
    RESULT_DIR = '/cluster/home/klehman/bnn/results'
else:
...
def generate_run_commands(command_list: List[str], num_cpus: int = 1, num_gpus: int = 0,
    ...
    if mode == 'euler':
        ...
        if promt:
            answer = input(
                f"About to launch {len(command_list)} commands in {num_cpus} local processes using {num_gpus} GPUs. Proceed? [yes/no]")
        else:
            answer = 'yes'
        ...
        if answer == 'yes':
            if dry:
                ...
                exec_ = AsyncExecutor(n_jobs=num_cpus)
                cmd_exec_fun = lambda cmd_: os.system(cmd_)
                exec_.run(cmd_exec_fun, command_list)
    else:
...
def sample_flag(sample_spec, rds=None):
    ...
    sample_type, range_ = sample_spec
    if sample_type == 'loguniform':
        assert len(range_) == 2
        return 10 ** rds.uniform(*range_)
    elif sample_type == 'uniform':
        assert len(range_) == 2
        return rds.uniform(*range_)
    elif sample_type == 'choice':
        return rds.choice(range_)
    else:
        raise NotImplementedError
        ...
        if os.path.isfile(results_file):
            try:
                ...
                if isinstance(exp_dict, dict):
                    exp_dict['evals'] = [dict([a, float(x)] for a, x in b.items()) for b in [exp_dict['evals']]][0]
